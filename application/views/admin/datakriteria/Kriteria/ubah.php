          ...
          <div class="card">
            ...
            <form method="post" action="<?= site_url('admin/DataKriteria/Kriteria/Kriteria/doEditKriteria/'.$detail['id_kriteria'])?>">
               <div class="form-group row">
                          <label class="col-sm-3 form-control-label">Nama Kepanitiaan</label>
                          <div class="col-sm-4">
                           <select class="form-control" name="id_kegiatan">
                            <option>-- Pilih Nama Kegiatan --</option>
                            <?php foreach ($select_option->result() as $row) {
                            ?>
                                <option value="<?= $row->id_kegiatan ?>"<?= ($row->id_kegiatan == $detail['id_kegiatan'] ? 'selected="selected"' : '') ?>><?= $row->nama_kegiatan?> </option>
